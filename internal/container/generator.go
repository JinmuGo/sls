...
func GenerateIncludeFile(cache *Cache, outputPath string) error {
	...
	// Iterate hosts in a stable order so the generated file is deterministic for
	// a given cache state (avoids spurious diffs for version-controlled configs).
	hostAliases := make([]string, 0, len(cache.Hosts))
	for hostAlias := range cache.Hosts {
		hostAliases = append(hostAliases, hostAlias)
	}
	sort.Strings(hostAliases)

	for _, hostAlias := range hostAliases {
		hc := cache.Hosts[hostAlias]
